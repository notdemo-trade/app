...
export const LLMCredentialSchema = z.object({
  apiKey: z.string().min(1),
  baseUrl: z.string().url().optional(), // for OpenAI-compatible proxies
})
export type LLMCredential = z.infer<typeof LLMCredentialSchema>

// Social — X API v2 (pay-per-use credits, app-only bearer token for search)
export const TwitterCredentialSchema = z.object({
  bearerToken: z.string().min(1),
})
export type TwitterCredential = z.infer<typeof TwitterCredentialSchema>

// Telegram bot for HITL approvals + notifications
