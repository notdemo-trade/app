...
import { usersListBindingQueryOptions } from '@/lib/query-keys';
...
export const Route = createFileRoute('/demo/binding/list')({
  component: BindingListPage,
  loader: async ({ context }) => {
    await context.queryClient.ensureQueryData(usersListBindingQueryOptions(defaultPagination));
  },
});
...
{`Browser Navigation
    ...
    ▼
Server Function (getUsersBinding)
    │
    │ 2. Zod validation
    ▼
env.DATA_SERVICE.fetch('https://data-service/users?limit=5&offset=0')
    │
    │ 3. Internal network call
    ▼
data-service (Hono API)
    │
    │ 4. authMiddleware → zValidator → userService.getUsers()
    ▼
...
// lib/query-keys.ts
export const usersListBindingQueryOptions = (params) =>
  queryOptions({
