...
import { z } from 'zod';
import { userDetailBindingQueryOptions } from '@/lib/query-keys';
...
export const Route = createFileRoute('/demo/binding/read')({
...
function BindingReadPage() {
  ...
  const handleSearch = (newId: string) => {
    navigate({ to: '/demo/binding/read', search: { userId: newId } });
  };
...
{`Browser Navigation
    ...
    ▼
Server Function (getUserBinding)
    │
    │ 2. Zod validation
    ▼
...
data-service (Hono API)
    │
    │ 4. authMiddleware → userService.getUser()
    ▼
...
// lib/query-keys.ts
export const userDetailBindingQueryOptions = (id) =>
  queryOptions({
