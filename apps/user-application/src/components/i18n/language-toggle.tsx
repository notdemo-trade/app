...
import { COOKIE_NAME, DEFAULT_LOCALE, isIgnoredPath, extractLocaleFromPath } from "@/i18n/core/shared"
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button variant={variant} size="default" className="aspect-square hover:scale-105 active:scale-95" aria-label={t("lang.select")}>
          <Languages className="h-4 w-4 text-foreground" />
        </Button>
      </DropdownMenuTrigger>
      <DropdownMenuContent align={align} className="w-44">
        {LOCALE_OPTIONS.map((opt) => (
          <DropdownMenuItem key={opt.value} onClick={() => setLocale(opt.value)} className="flex items-center gap-3 cursor-pointer">
